...
- **v0.4：** 落地发布证据状态词：`candidate`、`draft`、`final-ready`、`certified`，
  并要求用轻量 `docs/release-evidence/` manifest 记录证据；同时把语言策略改为英文正文 + 专业词对照表，并加入本地精确 SVG、内置 Kroki 专业图、外部复核信息图的三层视觉路线。这个说明不会把任何
  candidate 路线升级成交付级；v0.3 证据仍然只是历史事实，除非重新生成并写入 manifest。
